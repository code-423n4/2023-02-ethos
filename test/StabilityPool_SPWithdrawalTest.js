...
contract('StabilityPool - Withdrawal of stability deposit - Reward calculations', async accounts => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], whale, dec(100000, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100000, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(100000, 18)), whale, whale, { from: whale })
      ...
      const alice_ETHWithdrawn = th.getEventArgByName(txA, 'CollateralGainWithdrawn', '_collAmount').toString()
      const bob_ETHWithdrawn = th.getEventArgByName(txB, 'CollateralGainWithdrawn', '_collAmount').toString()
      const carol_ETHWithdrawn = th.getEventArgByName(txC, 'CollateralGainWithdrawn', '_collAmount').toString()
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '33166666666667'), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '33166666666667'), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '33166666666667'), 10000)
    })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(100, collDecimals))
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '66333333333333'), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '66333333333333'), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '66333333333333'), 10000)
    })
    ...
    it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(100, collDecimals))
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(100, collDecimals))
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_3, defaulter_3, { from: defaulter_3 })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(99500, 9)), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(99500, 9)), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(99500, 9)), 10000)
    })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing LUSD", async () => {
      ...
      // Defaulters open trove with 200% ICR
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(50, collDecimals))
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(70, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(50, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(5000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await borrowerOperations.openTrove(collaterals[0].address, dec(70, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(7000, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      ...
      // (0.5 + 0.7) * 99.5 / 3
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(398, 11)), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(398, 11)), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(398, 11)), 10000)
    })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing LUSD", async () => {
      ...
      // Defaulters open trove with 200% ICR
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(50, collDecimals))
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(60, collDecimals))
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(70, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(50, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(5000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await borrowerOperations.openTrove(collaterals[0].address, dec(60, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(6000, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      await borrowerOperations.openTrove(collaterals[0].address, dec(70, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(7000, 18)), defaulter_3, defaulter_3, { from: defaulter_3 })
      ...
      // (0.5 + 0.6 + 0.7) * 99.5 / 3
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(597, 11)), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(597, 11)), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(597, 11)), 10000)
    })
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '33166666666667'), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '66333333333333'), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(995, 11)), 100000)
    })
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '49750000000000'), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(995, 11)), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '149250000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], whale, dec(1000000, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(1000000, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(1000000, 18)), whale, whale, { from: whale })
      ...
      */
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(2160, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(2160, collDecimals), th._100pct, await getOpenTroveLUSDAmount('207000000000000000000000'), defaulter_1, defaulter_1, { from: defaulter_1 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(50, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(50, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(5, 21)), defaulter_2, defaulter_2, { from: defaulter_2 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(500, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(500, collDecimals), th._100pct, await getOpenTroveLUSDAmount('46700000000000000000000'), defaulter_3, defaulter_3, { from: defaulter_3 })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '11447463383570'), 1000000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '2610021651454043'), 1000000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '74980885162385'), 1000000)
    })
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      const bob_ETHWithdrawn = th.getEventArgByName(txB, 'CollateralGainWithdrawn', '_collAmount').toString()
      const carol_ETHWithdrawn = th.getEventArgByName(txC, 'CollateralGainWithdrawn', '_collAmount').toString()
      const dennis_ETHWithdrawn = th.getEventArgByName(txD, 'CollateralGainWithdrawn', '_collAmount').toString()
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '82916666666667'), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '82916666666667'), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '82916666666667'), 10000)

      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '49750000000000'), 10000)
    })
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_3, defaulter_3, { from: defaulter_3 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_4, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_4, defaulter_4, { from: defaulter_4 })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(995, 11)), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(995, 11)), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(995, 11)), 10000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, dec(995, 11)), 10000)
    })
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      */
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(250, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(250, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(25000, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(50, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(50, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(5000, 18)), defaulter_3, defaulter_3, { from: defaulter_3 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_4, dec(400, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(400, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(40000, 18)), defaulter_4, defaulter_4, { from: defaulter_4 })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '419563467492260'), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '139854489164086'), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '104890866873065'), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '131691176470588'), 100000)
    })
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      */
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(300, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(300, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(30000, 18)), defaulter_3, defaulter_3, { from: defaulter_3 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_4, dec(50, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(50, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(5000, 18)), defaulter_4, defaulter_4, { from: defaulter_4 })
      ...
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '122461538461538'), 100000)
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '182361204013377'), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '227951505016722'), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '113975752508361'), 100000)
    })
    ...
    it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 LUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '82916666666667'), 100000)
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '92866666666667'), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '92866666666667'), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '79600000000000'), 10000)
    })
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // await borrowerOperations.openTrove(th._100pct, dec(1, 18), account, account, { from: erin, value: dec(2, collDecimals) })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(995, 11)), 100000)
      ...
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '49750000000000'), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '49750000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const carol_ETHWithdrawn = th.getEventArgByName(txC, 'CollateralGainWithdrawn', '_collAmount').toString()
      const dennis_ETHWithdrawn = th.getEventArgByName(txD, 'CollateralGainWithdrawn', '_collAmount').toString()
      const erin_ETHWithdrawn = th.getEventArgByName(txE, 'CollateralGainWithdrawn', '_collAmount').toString()
      ...
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '16583333333333'), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '33166666666667'), 100000)
      assert.isAtMost(th.getDifference(erin_ETHWithdrawn, '49750000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const dennis_ETHWithdrawn = th.getEventArgByName(txD, 'CollateralGainWithdrawn', '_collAmount').toString()
      const erin_ETHWithdrawn = th.getEventArgByName(txE, 'CollateralGainWithdrawn', '_collAmount').toString()
      const flyn_ETHWithdrawn = th.getEventArgByName(txF, 'CollateralGainWithdrawn', '_collAmount').toString()
      const graham_ETHWithdrawn = th.getEventArgByName(txG, 'CollateralGainWithdrawn', '_collAmount').toString()
      const harriet_ETHWithdrawn = th.getEventArgByName(txH, 'CollateralGainWithdrawn', '_collAmount').toString()
      ...
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(erin_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(flyn_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(graham_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(harriet_ETHWithdrawn, dec(995, 11)), 100000)
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount('9999999910000000000000'), defaulter_1, defaulter_1, { from: defaulter_1 })

      assert.equal(await stabilityPool.currentScale(), '0')

      // Defaulter 2 withdraws 9900 LUSD
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(60, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(60, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(9900, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      ...
      // Grab the ETH gain from the emitted event in the tx log
      const alice_ETHWithdrawn = await th.getEventArgByName(txA, 'CollateralGainWithdrawn', '_collAmount').toString()
      ...
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
      const bob_ETHWithdrawn = await th.getEventArgByName(txB, 'CollateralGainWithdrawn', '_collAmount').toString()
      ...
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '59700000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 2 withdraws 54450 LUSD
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(330, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(330, collDecimals), th._100pct, await getOpenTroveLUSDAmount('54450000000000000000000'), defaulter_2, defaulter_2, { from: defaulter_2 })
      ...
      // 54450 LUSD liquidated.  P altered by a factor of 1-(54450/60000) = 0.0925. Scale changed.
      ...
      /* Expect depositors to withdraw 9.25% of their initial deposit, and an ETH gain 
      in proportion to their initial deposit:
     
      Bob:  925 LUSD, 55 Ether
      Carol:  1850 LUSD, 110 Ether
      Dennis:  2775 LUSD, 165 Ether
     
      Total: 6000 LUSD, 300 Ether
      */
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(925, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(1850, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), dec(2775, 18)), 100000)

      const bob_ETHWithdrawn = await th.getEventArgByName(txB, 'CollateralGainWithdrawn', '_collAmount').toString()
      const carol_ETHWithdrawn = await th.getEventArgByName(txC, 'CollateralGainWithdrawn', '_collAmount').toString()
      const dennis_ETHWithdrawn = await th.getEventArgByName(txD, 'CollateralGainWithdrawn', '_collAmount').toString()

      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '54725000000000'), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '109450000000000'), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '164175000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(99999, 17)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(99999, 17)), defaulter_2, defaulter_2, { from: defaulter_2 })
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount('9999900000000000000000'), defaulter_1, defaulter_1, { from: defaulter_1 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(600, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(600, collDecimals), th._100pct, await getOpenTroveLUSDAmount('59999400000000000000000'), defaulter_2, defaulter_2, { from: defaulter_2 })
      ...
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(995, 11)), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(1990, 11)), 1000000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, dec(2985, 11)), 1000000)
    })
    ...
    it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount('9999999999900000000000'), defaulter_1, defaulter_1, { from: defaulter_1 })
    ...
    */
    it("withdrawFromSP(): Several deposits of 10000 LUSD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount('9999900000000000000000'), defaulter_1, defaulter_1, { from: defaulter_1 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount('9999900000000000000000'), defaulter_2, defaulter_2, { from: defaulter_2 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount('9999900000000000000000'), defaulter_3, defaulter_3, { from: defaulter_3 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_4, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount('9999900000000000000000'), defaulter_4, defaulter_4, { from: defaulter_4 })
      ...
      assert.isTrue(toBN(alice_ETHWithdrawn).sub(toBN(dec(995, 11))).abs().lte(toBN(dec(1, 11))))
      assert.isTrue(toBN(bob_ETHWithdrawn).sub(toBN(dec(995, 11))).abs().lte(toBN(dec(1, 11))))
      assert.isTrue(toBN(carol_ETHWithdrawn).sub(toBN(dec(995, 11))).abs().lte(toBN(dec(1, 11))))
      assert.isTrue(toBN(dennis_ETHWithdrawn).sub(toBN(dec(995, 11))).abs().lte(toBN(dec(1, 11))))
    })
    ...
    it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(200, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(200, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(24100, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_2, dec(200, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(200, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(24300, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(200, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(200, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(24500, 18)), defaulter_3, defaulter_3, { from: defaulter_3 })
    ...
    it("withdrawFromSP(): Depositor's collateral gain stops increasing after two scale changes", async () => {
      ...
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(99999, 17)), defaulter_2, defaulter_2, { from: defaulter_2 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_3, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(99999, 17)), defaulter_3, defaulter_3, { from: defaulter_3 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_4, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(99999, 17)), defaulter_4, defaulter_4, { from: defaulter_4 })
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_5, dec(100, collDecimals))
      await borrowerOperations.openTrove(collaterals[0].address, dec(100, collDecimals), th._100pct, await getOpenTroveLUSDAmount(dec(99999, 17)), defaulter_5, defaulter_5, { from: defaulter_5 })
      ...
      const alice_ETHGainAt2ndScaleChange = (await stabilityPool.getDepositorCollateralGain(alice))[1][0].toString()
      ...
      const alice_ETHGainAfterFurtherLiquidation = (await stabilityPool.getDepositorCollateralGain(alice))[1][0].toString()
    ...
    it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      const depositors = [alice, bob]
      for (account of depositors) {
        await mintCollateralAndApproveBorrowerOps(collaterals[0], account, dec(2, 27))
        await borrowerOperations.openTrove(collaterals[0].address, dec(2, 27), th._100pct, dec(1, 36), account, account, { from: account })
        await stabilityPool.provideToSP(dec(1, 36), { from: account })
      }

      // Defaulter opens trove with 200% ICR
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, dec(1, 21))
      await borrowerOperations.openTrove(collaterals[0].address, dec(1, 21), th._100pct, await getOpenTroveLUSDAmount(dec(1, 36)), defaulter_1, defaulter_1, { from: defaulter_1 })
      ...
      // Grab the ETH gain from the emitted event in the tx log
      const alice_ETHWithdrawn = th.getEventArgByName(txA, 'CollateralGainWithdrawn', '_collAmount')
      const bob_ETHWithdrawn = th.getEventArgByName(txB, 'CollateralGainWithdrawn', '_collAmount')
      ...
      // Check ETH gains
      const aliceExpectedETHGain = toBN(dec(4975, 17))
      const aliceETHDiff = aliceExpectedETHGain.sub(toBN(alice_ETHWithdrawn))

      console.log(alice_ETHWithdrawn.toString());
      assert.isTrue(aliceETHDiff.lte(toBN(dec(1, 18))))

      const bobExpectedETHGain = toBN(dec(4975, 17))
    ...
    it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      const depositors = [alice, bob]
      for (account of depositors) {
        await mintCollateralAndApproveBorrowerOps(collaterals[0], account, dec(2, 29))
        await borrowerOperations.openTrove(collaterals[0].address, dec(2, 29), th._100pct, dec(1, 38), account, account, { from: account })
        await stabilityPool.provideToSP(dec(1, 38), { from: account })
      }

      // Defaulter opens trove with 50e-7 ETH and  5000 LUSD. 200% ICR
      await mintCollateralAndApproveBorrowerOps(collaterals[0], defaulter_1, '5000000')
      await borrowerOperations.openTrove(collaterals[0].address, '5000000', th._100pct, await getOpenTroveLUSDAmount(dec(5000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })

...
contract('LUSDToken', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      const contracts = await deploymentHelper.deployTestCollaterals(await deploymentHelper.deployTesterContractsHardhat())
    ...
    it("name(): returns the token's name", async () => {
      ...
      assert.equal(name, "Ethos Reserve Note")
    })

    it("symbol(): returns the token's symbol", async () => {
      const symbol = await lusdTokenTester.symbol()
      assert.equal(symbol, "ERN")
    })

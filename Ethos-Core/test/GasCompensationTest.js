...
contract('Gas compensation tests', async accounts => {
  ...
  it('_getCollGasCompensation(): returns the 0.5% of collaterall if it is < $10 in value', async () => {
    ...
    // const price_1 = await priceFeed.getPrice(collaterals[0].address)
    ...
    -> Expect 0.5% of collaterall as gas compensation */
    await priceFeed.setPrice(collaterals[0].address, '28400000000000000000')
    // const price_2 = await priceFeed.getPrice(collaterals[0].address)
    ...
    // const price_3 = await priceFeed.getPrice(collaterals[0].address)
  ...
  it('getCollGasCompensation(): returns 0.5% of collaterall when 0.5% of collateral = $10 in value', async () => {
    ...
    await priceFeed.setPrice(collaterals[0].address, '45323545420000000000000')
    ...
    await priceFeed.setPrice(collaterals[0].address, dec(1, 24))
    const price_2 = await priceFeed.getPrice(collaterals[0].address)
  ...
  it('getCompositeDebt(): returns (debt + 10) when 0.5% of collateral > $10 in value', async () => {
    ...
    coll = 100 ETH  
    debt = 2000 LUSD
    -> Expect composite debt = (2000 + 10) = 2010 LUSD  */
   ...
   -> Expect composite debt = (54321.123456789 + 100) = 54331.123456789 LUSD */
    await priceFeed.setPrice(collaterals[0].address, dec(1, 24))
    const price_3 = await priceFeed.getPrice(collaterals[0].address)
  ...
  it('getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves', async () => {
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(200, 18)), extraParams: { from: whale } })
    const price = await priceFeed.getPrice(collaterals[0].address)

    // A opens with 1 ETH, 110 LUSD
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(18, 17)), extraParams: { from: alice } })
    ...
    assert.isAtMost(th.getDifference(alice_ICR, toBN(dec(18, 17))), 1000)
    ...
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(100, 18), extraParams: { from: flyn } })
    const flyn_ICR = (await troveManager.getCurrentICR(flyn, collaterals[0].address, price)).toString()
    // Expect Flyn's ICR = (1 * 200) / 100 = 200%
    assert.isAtMost(th.getDifference(flyn_ICR, dec(2, 18)), 1000)

    // C opens with 2.5 ETH, 160 LUSD
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(3125, 15)), extraParams: { from: carol } })
    const carol_ICR = (await troveManager.getCurrentICR(carol, collaterals[0].address, price)).toString()
    ...
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(4, 18)), extraParams: { from: dennis } })
    ...
    await openTrove({ collateral: collaterals[0], ICR: toBN('27028668628933700000'), extraParams: { from: erin } })
    const erin_ICR = (await troveManager.getCurrentICR(erin, collaterals[0].address, price)).toString()
    // Expect Erin's ICR = (4405.45 * 200) / (32598.35) = 2702.87%
    assert.isAtMost(th.getDifference(erin_ICR, '27028668628933700000'), 10**7)

    // H opens with 1 ETH, 180 LUSD
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(12, 17)), extraParams: { from: harriet } })
    const harriet_ICR = (await troveManager.getCurrentICR(harriet, collaterals[0].address, price)).toString()
    // Expect Harriet's ICR = (1 * 200) / (180) = 111.11%
    assert.isAtMost(th.getDifference(harriet_ICR, toBN(dec(12, 17))), 1000)
  })
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(20000, 18)), extraParams: { from: whale } })

    // A-E open troves
    const { totalDebt: A_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(200, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: B_totalDebt.add(C_totalDebt), extraParams: { from: erin } })
    ...
    // --- Price drops to 9.99 ---
    await priceFeed.setPrice(collaterals[0].address, '9990000000000000000')
    const price_1 = await priceFeed.getPrice(collaterals[0].address)
    ...
    const aliceColl = (await troveManager.Troves(alice, collaterals[0].address))[1]
    ...
    const liquidatorBalance_before_A = await collaterals[0].balanceOf(liquidator)
    const A_GAS_Used_Liquidator = th.gasUsed(await troveManager.liquidate(alice, collaterals[0].address, { from: liquidator, gasPrice: GAS_PRICE }))
    const liquidatorBalance_after_A = await collaterals[0].balanceOf(liquidator)
    ...
    assert.equal(compensationReceived_A.toString(), _0pt5percent_aliceColl.toString())
    ...
    const ETHinSP_A = await stabilityPool.getCollateral(collaterals[0].address)
    assert.equal(ETHinSP_A.toString(), aliceColl.sub(_0pt5percent_aliceColl)) // 1 ETH - 0.5%

    // --- Price drops to 3 ---
    await priceFeed.setPrice(collaterals[0].address, dec(3, 18))
    const price_2 = await priceFeed.getPrice(collaterals[0].address)
    ...
    // Check collateral value in USD is < $10
    const bobColl = (await troveManager.Troves(bob, collaterals[0].address))[1]

    assert.isFalse(await th.checkRecoveryMode(contracts, collaterals[0].address))
    // Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidatorBalance_before_B = await collaterals[0].balanceOf(liquidator)
    const B_GAS_Used_Liquidator = th.gasUsed(await troveManager.liquidate(bob, collaterals[0].address, { from: liquidator, gasPrice: GAS_PRICE }))
    const liquidatorBalance_after_B = await collaterals[0].balanceOf(liquidator)
    ...
    assert.equal(compensationReceived_B.toString(), _0pt5percent_bobColl.toString()) // 0.5% of 2 ETH
    ...
    // Check ETH in SP has received the liquidation
    const ETHinSP_B = await stabilityPool.getCollateral(collaterals[0].address)
    assert.equal(ETHinSP_B.toString(), aliceColl.sub(_0pt5percent_aliceColl).add(bobColl).sub(_0pt5percent_bobColl)) // (1 + 2 ETH) * 0.995


    // --- Price drops to 3 ---
    await priceFeed.setPrice(collaterals[0].address, '3141592653589793238')
    const price_3 = await priceFeed.getPrice(collaterals[0].address)
    ...
    // Check collateral value in USD is < $10
    const carolColl = (await troveManager.Troves(carol, collaterals[0].address))[1]

    assert.isFalse(await th.checkRecoveryMode(contracts, collaterals[0].address))
    // Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidatorBalance_before_C = await collaterals[0].balanceOf(liquidator)
    const C_GAS_Used_Liquidator = th.gasUsed(await troveManager.liquidate(carol, collaterals[0].address, { from: liquidator, gasPrice: GAS_PRICE }))
    const liquidatorBalance_after_C = await collaterals[0].balanceOf(liquidator)
    ...
    assert.equal(compensationReceived_C.toString(), _0pt5percent_carolColl.toString())
    ...
    const ETHinSP_C = await stabilityPool.getCollateral(collaterals[0].address)
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2000, 18)), extraParams: { from: whale } })

    // A-E open troves
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(200, 18), extraParams: { from: alice } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(120, 16)), extraLUSDAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(60, 18)), extraLUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(80, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(80, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: erin } })
    ...
    const LUSDinSP_0 = await stabilityPool.getTotalLUSDDeposits()
    const ETHinSP_0 = await stabilityPool.getCollateral(collaterals[0].address)

    // --- Price drops to 199.999 ---
    await priceFeed.setPrice(collaterals[0].address, '199999000000000000000')
    const price_1 = await priceFeed.getPrice(collaterals[0].address)
    ...
    const aliceICR = await troveManager.getCurrentICR(alice, collaterals[0].address, price_1)
    ...
    // --- Price drops to 15 ---
    await priceFeed.setPrice(collaterals[0].address, dec(15, 18))
    const price_2 = await priceFeed.getPrice(collaterals[0].address)
    ...
    const bobICR = await troveManager.getCurrentICR(bob, collaterals[0].address, price_2)
    ...
    const _0pt5percent_bobColl = bobColl.div(web3.utils.toBN('200'))
    const compensationReceived_B = liquidatorBalance_after_B.sub(liquidatorBalance_before_B)
    assert.equal(compensationReceived_B.toString(), _0pt5percent_bobColl.toString())
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(2000, 18), extraParams: { from: alice,} })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(1875, 15)), extraLUSDAmount: dec(8000, 18), extraParams: { from: bob,} })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(600, 18), extraParams: { from: carol} })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(4, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: dennis} })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(4, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: erin} })
   ...
   and (37.5 - 0.1875 ETH) ETH remainder liquidated */
    ...
    const bobICR = await troveManager.getCurrentICR(bob, collaterals[0].address, price_1)
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(2, 18)), extraLUSDAmount: B_totalDebt, extraParams: { from: erin } })
    ...
    // th.logBN('TCR', await troveManager.getTCR(await priceFeed.getPrice(collaterals[0].address)))
    // --- Price drops to 9.99 ---
    await priceFeed.setPrice(collaterals[0].address, '9990000000000000000')
    const price_1 = await priceFeed.getPrice(collaterals[0].address)
    ...
    // Check collateral value in USD is < $10
    const aliceColl = (await troveManager.Troves(alice, collaterals[0].address))[1]
    const aliceDebt = (await troveManager.Troves(alice, collaterals[0].address))[0]

    // th.logBN('TCR', await troveManager.getTCR(await priceFeed.getPrice(collaterals[0].address)))
    assert.isFalse(await th.checkRecoveryMode(contracts, collaterals[0].address))

    // Liquidate A (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidationTxA = await troveManager.liquidate(alice, collaterals[0].address, { from: liquidator, gasPrice: GAS_PRICE })
    ...
    // Check collateral value in USD is < $10
    const bobColl = (await troveManager.Troves(bob, collaterals[0].address))[1]
    const bobDebt = (await troveManager.Troves(bob, collaterals[0].address))[0]

    assert.isFalse(await th.checkRecoveryMode(contracts, collaterals[0].address))
    // Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidationTxB = await troveManager.liquidate(bob, collaterals[0].address, { from: liquidator, gasPrice: GAS_PRICE })
  ...
  it('gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    console.log(price_1.toString())
    console.log(collDecimals.toString())
    const aliceCollValueInUSD = (await borrowerOperationsTester.getUSDValue(aliceColl, price_1, collDecimals))
  ...
  it('liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    // A-F open troves
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(128, 16)), extraLUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(526, 16)), extraLUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(488, 16)), extraLUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(545, 16)), extraLUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(10, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(10, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    assert.isTrue((await troveManager.getCurrentICR(alice, collaterals[0].address, price)).lt(mv._MCR))
    assert.isTrue((await troveManager.getCurrentICR(bob, collaterals[0].address, price)).lt(mv._MCR))
    assert.isTrue((await troveManager.getCurrentICR(carol, collaterals[0].address, price)).lt(mv._MCR))
    assert.isTrue((await troveManager.getCurrentICR(dennis, collaterals[0].address, price)).lt(mv._MCR))

    // Check E, F have ICR > MCR
    assert.isTrue((await troveManager.getCurrentICR(erin, collaterals[0].address, price)).gt(mv._MCR))
    assert.isTrue((await troveManager.getCurrentICR(flyn, collaterals[0].address, price)).gt(mv._MCR))
    ...
    const bobColl = (await troveManager.Troves(bob, collaterals[0].address))[1]
    const carolColl = (await troveManager.Troves(carol, collaterals[0].address))[1]
    const dennisColl = (await troveManager.Troves(dennis, collaterals[0].address))[1]
    ...
    */
    ...
    const liquidatorBalance_before = await collaterals[0].balanceOf(liquidator)
    const GAS_Used_Liquidator = th.gasUsed(await troveManager.liquidateTroves(collaterals[0].address, 4, { from: liquidator, gasPrice: GAS_PRICE }))
    const liquidatorBalance_after = await collaterals[0].balanceOf(liquidator)
    ...
    const compensationReceived = liquidatorBalance_after.sub(liquidatorBalance_before)
    assert.equal(expectedGasComp.toString(), compensationReceived.toString())
  ...
  it('liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    const LUSDinDefaultPool_0 = await defaultPool.getLUSDDebt(collaterals[0].address)
    ...
    */
    ...
    const LUSDinDefaultPool_1 = await defaultPool.getLUSDDebt(collaterals[0].address)
    ...
    const ETHinDefaultPool = (await defaultPool.getCollateral(collaterals[0].address)).toString()
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    // A-F open troves
    const { totalDebt: A_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(128, 16)), extraLUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(526, 16)), extraLUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(488, 16)), extraLUSDAmount: dec(600, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ collateral: collaterals[0], ICR: toBN(dec(545, 16)), extraLUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(10, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ collateral: collaterals[0], ICR: toBN(dec(10, 18)), extraLUSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Liquidate troves A-D
    const liquidationTxData = await troveManager.liquidateTroves(collaterals[0].address, 4, { from: liquidator, gasPrice: GAS_PRICE })
  ...
  it('Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    for (const account of _10_accounts) {
      ...
      await openTrove({ collateral: collaterals[0], value: toBN(dec(30, 'ether')), extraLUSDAmount: debtString, extraParams: { from: account } })
    ...
    const initialPrice = await priceFeed.getPrice(collaterals[0].address)
    const firstColl = (await troveManager.Troves(_10_accounts[0], collaterals[0].address))[1]
    ...
    while (price < 210) {
      ...
      await priceFeed.setPrice(collaterals[0].address, priceString)

      const ICRList = []
      const coll_firstTrove = (await troveManager.Troves(_10_accounts[0], collaterals[0].address))[1]
      ...
      for (account of _10_accounts) {
        ...
        const coll = (await troveManager.Troves(account, collaterals[0].address))[1]
        ...
        const ICR = await troveManager.getCurrentICR(account, collaterals[0].address, price)
        ICRList.push(ICR)
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    for (const account of _20_accounts) {
      ...
      await openTrove({ collateral: collaterals[0], value: toBN(collString), extraLUSDAmount: dec(100, 18), extraParams: { from: account } })

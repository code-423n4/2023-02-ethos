 ...
 // Sequentially add coll and withdraw LUSD, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { collateral: collaterals[0], extraLUSDAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
  ...
  it("setup: makes accounts with nominal ICRs increasing by 1% consecutively", async () => {
    ...
    const ICR_0 = await troveManager.getNominalICR(accounts[0], collaterals[0].address)
    const ICR_1 = await troveManager.getNominalICR(accounts[1], collaterals[0].address)
    const ICR_2 = await troveManager.getNominalICR(accounts[2], collaterals[0].address)
    const ICR_3 = await troveManager.getNominalICR(accounts[3], collaterals[0].address)
    const ICR_4 = await troveManager.getNominalICR(accounts[4], collaterals[0].address)
    const ICR_5 = await troveManager.getNominalICR(accounts[5], collaterals[0].address)
    const ICR_6 = await troveManager.getNominalICR(accounts[6], collaterals[0].address)
    const ICR_7 = await troveManager.getNominalICR(accounts[7], collaterals[0].address)
    const ICR_8 = await troveManager.getNominalICR(accounts[8], collaterals[0].address)
    const ICR_9 = await troveManager.getNominalICR(accounts[9], collaterals[0].address)
  ...
  it("getApproxHint(): returns the address of a Trove within sqrt(length) positions of the correct insert position", async () => {
    ...
    ;({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(collaterals[0].address, CR_250, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_250 = await troveManager.getNominalICR(hintAddress, collaterals[0].address)
    ...
    ;({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(collaterals[0].address, CR_287, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_287 = await troveManager.getNominalICR(hintAddress, collaterals[0].address)
    ...
    ;({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(collaterals[0].address, CR_213, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_213 = await troveManager.getNominalICR(hintAddress, collaterals[0].address)
    ...
    //  const hintAddress_201 = await functionCaller.troveManager_getApproxHint(CR_201, sqrtLength * 10)
     ;({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(collaterals[0].address, CR_201, sqrtLength * 10, latestRandomSeed))
     const ICR_hintAddress_201 = await troveManager.getNominalICR(hintAddress, collaterals[0].address)
  ...
  it("getApproxHint(): returns the head of the list if the CR is the max uint256 value", async () => {
    ...
    ;({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(collaterals[0].address, CR_Max, sqrtLength * 10, latestRandomSeed))

    const ICR_hintAddress_Max = await troveManager.getNominalICR(hintAddress, collaterals[0].address)
    const ICRPercent_hintAddress_Max = Number(web3.utils.fromWei(ICR_hintAddress_Max, 'ether')) * 100

     const firstTrove = await sortedTroves.getFirst(collaterals[0].address)
     const ICR_FirstTrove = await troveManager.getNominalICR(firstTrove, collaterals[0].address)
  ...
  it("getApproxHint(): returns the tail of the list if the CR is lower than ICR of any Trove", async () => {
    ...
    ;({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(collaterals[0].address, CR_Min, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_Min = await troveManager.getNominalICR(hintAddress, collaterals[0].address)
    const ICRPercent_hintAddress_Min = Number(web3.utils.fromWei(ICR_hintAddress_Min, 'ether')) * 100

     const lastTrove = await sortedTroves.getLast(collaterals[0].address)
     const ICR_LastTrove = await troveManager.getNominalICR(lastTrove, collaterals[0].address)
  ...
  it('computeNominalCR()', async () => {
    ...
    const NICR = await hintHelpers.computeNominalCR(dec(3, collDecimals), dec(200, 18), collDecimals)

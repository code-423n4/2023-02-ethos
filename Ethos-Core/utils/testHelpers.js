...
class TestHelper {
  ...
  static async ICRbetween100and110(account, collAddress, troveManager, price) {
    const ICR = await troveManager.getCurrentICR(account, collAddress, price)
  ...
  static normalizeColl18Dedc(unwrappedColl, collDecimals) {
    const bn_18 = this.toBN(18)
    let normalizedColl = unwrappedColl
    if (collDecimals.gt(bn_18)) {
      normalizedColl = normalizedColl.div(this.toBN(10).pow(collDecimals.sub(bn_18)))
    } else if (collDecimals.lt(bn_18)) {
      normalizedColl = normalizedColl.mul(this.toBN(10).pow(bn_18.sub(collDecimals)))
    }
    return normalizedColl
  }

  static unwrapCollNativeDec(normalizedColl, collDecimals) {
    const bn_18 = this.toBN(18)
    let unwrappedColl = normalizedColl
    if (collDecimals.gt(bn_18)) {
      unwrappedColl = unwrappedColl.mul(this.toBN(10).pow(collDecimals.sub(bn_18)))
    } else if (collDecimals.lt(bn_18)) {
      unwrappedColl = unwrappedColl.div(this.toBN(10).pow(bn_18.sub(collDecimals)))
    }
    return unwrappedColl
  }
  // --- Logging functions ---
  ...
  static async checkRecoveryMode(contracts, collAddress) {
    const price = await contracts.priceFeedTestnet.getPrice(collAddress)
    return contracts.troveManager.checkRecoveryMode(collAddress, price)
  }

  static async getTCR(contracts, collAddress) {
    const price = await contracts.priceFeedTestnet.getPrice(collAddress)
    return contracts.troveManager.getTCR(collAddress, price)
  }
  ...
  static async getTroveEntireColl(contracts, trove, collAddress) {
    return this.toBN((await contracts.troveManager.getEntireDebtAndColl(trove, collAddress))[1])
  }

  static async getTroveEntireDebt(contracts, trove, collAddress) {
    return this.toBN((await contracts.troveManager.getEntireDebtAndColl(trove, collAddress))[0])
  }

  static async getTroveStake(contracts, trove, collAddress) {
    return (contracts.troveManager.getTroveStake(trove, collAddress))
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        ...
        const LUSDAmount = redemptionTx.logs[i].args[1]
        const totalLUSDRedeemed = redemptionTx.logs[i].args[2]
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const lusdGasComp = liquidationTx.logs[i].args[4]
  ...
  static async getEntireCollAndDebt(contracts, account, collAddress) {
    // console.log(`account: ${account}`)
    const rawColl = (await contracts.troveManager.Troves(account, collAddress))[1]
    const rawDebt = (await contracts.troveManager.Troves(account, collAddress))[0]
    const pendingCollateralReward = await contracts.troveManager.getPendingCollateralReward(account, collAddress)
    const pendingLUSDDebtReward = await contracts.troveManager.getPendingLUSDDebtReward(account, collAddress)
  ...
  static async openTrove(contracts, {
    ...
    if (!ICR && !value) ICR = await contracts.collateralConfig.getCollateralCCR(collateral.address);
    else if (typeof ICR == 'string') ICR = this.toBN(ICR)

    const totalDebt = await this.getOpenTroveTotalDebt(contracts, lusdAmount)
    const netDebt = await this.getActualDebtFromComposite(totalDebt, contracts)

    let price = await contracts.priceFeedTestnet.getPrice(collateral.address)
    if (price.eq(MoneyValues._zeroBN)) {
      await contracts.priceFeedTestnet.setPrice(collateral.address, MoneyValues._100e18BN.mul(this.toBN(2)));
      price = await contracts.priceFeedTestnet.getPrice(collateral.address)
    }

    let collAmount = value;
    if (ICR) {
      const collDecimals = await contracts.collateralConfig.getCollateralDecimals(collateral.address)
      collAmount = ICR.mul(totalDebt).div(price).div(this.toBN(10).pow(this.toBN(18).sub(collDecimals)))
    }

    // top-up sender's collateral balance if needed
    let senderCollBalance = await collateral.balanceOf(extraParams.from);
    if (senderCollBalance.lt(collAmount)) {
      await collateral.mint(extraParams.from, collAmount.sub(senderCollBalance))
    }

    // approve borrowerOps
    let spenderAddress = contracts.borrowerOperations.address
    if (contracts.borrowerOperations.proxies) {
      spenderAddress = contracts.borrowerOperations.proxies[extraParams.from].address
    }
    await collateral.approveInternal(extraParams.from, spenderAddress, collAmount);

    const tx = await contracts.borrowerOperations.openTrove(collateral.address, collAmount, maxFeePercentage, lusdAmount, upperHint, lowerHint, extraParams)
  ...
  static async withdrawLUSD(contracts, {
    ...
    if (ICR) {
      ...
      const { debt, coll } = await contracts.troveManager.getEntireDebtAndColl(extraParams.from, collateral.address)
      const price = await contracts.priceFeedTestnet.getPrice(collateral.address)
      const collDecimals = await contracts.collateralConfig.getCollateralDecimals(collateral.address)
      const targetDebt = this.normalizeColl18Dedc(coll, collDecimals).mul(price).div(ICR)
    ...
    await contracts.borrowerOperations.withdrawLUSD(collateral.address, maxFeePercentage, lusdAmount, upperHint, lowerHint, extraParams)
  ...
  static async redeemCollateral(redeemer, collAddress, contracts, LUSDAmount, gasPrice = 0, maxFee = this._100pct) {
    const price = await contracts.priceFeedTestnet.getPrice(collAddress)
    const tx = await this.performRedemptionTx(redeemer, collAddress, price, contracts, LUSDAmount, maxFee, gasPrice)
  ...
  static async performRedemptionTx(redeemer, collAddress, price, contracts, LUSDAmount, maxFee = 0, gasPrice_toUse = 0) {
    const redemptionhint = await contracts.hintHelpers.getRedemptionHints(collAddress, LUSDAmount, price, gasPrice_toUse)
    ...
    const {
      ...
      latestRandomSeed
    } = await contracts.hintHelpers.getApproxHint(collAddress, partialRedemptionNewICR, 50, this.latestRandomSeed)
    this.latestRandomSeed = latestRandomSeed

    const exactPartialRedemptionHint = (await contracts.sortedTroves.findInsertPosition(collAddress, partialRedemptionNewICR,
      approxPartialRedemptionHint,
      approxPartialRedemptionHint))

    const tx = await contracts.troveManager.redeemCollateral(collAddress, LUSDAmount,
      firstRedemptionHint,
